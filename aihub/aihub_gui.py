...
class MyApp:
    def __init__(self, root):
        ...
        self.text_area.grid(
            row=1, column=0, columnspan=4, sticky="nsew", padx=10, pady=10
        )

        # Header label for the chat
        self.header_label = tk.Label(
            root, text="Message", font=("Helvetica", 16, "bold")
        )
        ...
        self.input_entry = scrolledtext.ScrolledText(
            root, wrap=tk.WORD, width=40, height=4
        )
        ...
        self.start_stop_button = tk.Button(
            root, text="Start aiHub lurking...", command=self.toggle_background_app
        )
        self.start_stop_button.grid(row=4, column=0, pady=10)

        # Button to clear the text area
        self.clear_button = tk.Button(
            root, text="Clear Text", command=self.clear_text_area
        )
        self.clear_button.grid(row=4, column=1, pady=10)

        # Canvas for the status "light"
        self.status_light_canvas = tk.Canvas(
            root, width=15, height=15, highlightthickness=0
        )
        self.status_light_canvas.grid(row=4, column=4, pady=10, padx=10)

        # Initialize the circle on the canvas
        self.status_light_circle = self.status_light_canvas.create_oval(
            1, 1, 15, 15, fill="red", outline=""
        )
        ...
        file_menu.add_command(
            label="Configuration", command=self.open_configuration_window
        )
    ...
    def open_configuration_window(self):
        ...
        api_entry = tk.Entry(
            config_window, textvariable=self.configuration_settings["api"]
        )
        ...
        prompt_entry = tk.Entry(
            config_window, textvariable=self.configuration_settings["prompt_prefix"]
        )
        ...
        save_button = tk.Button(
            ...
            command=lambda: self.save_configuration(
                api_entry.get(), prompt_entry.get(), config_window
            ),
        )
    ...
    def show_about(self):
        ...
        try:
            ...
            readme_text = scrolledtext.ScrolledText(
                about_window, wrap=tk.WORD, width=60, height=20
            )
    ...
    def stop_background_app(self):
        ...
        self.start_stop_button.config(
            text="Start aiHub lurking...", command=self.start_background_app
        )
        # Update the flag
        self.background_app_running = False

        # Update the status "light" color
        self.status_light_canvas.itemconfig(
            self.status_light_circle, fill="red", outline=""
        )
    ...
    def start_background_app(self):
        if not self.background_app_running:
            ...
            self.start_stop_button.config(
                text="Stop aiHub", command=self.stop_background_app
            )
            ...
            # Update the status "light" color
            self.status_light_canvas.itemconfig(
                self.status_light_circle, fill="green", outline=""
            )
        ...
        try:
            ...
            self.process = subprocess.Popen(
                [
                    ...
                    "-gui",
                    "-api",
                    self.configuration_settings["api"].get(),
                    "-pp",
                    self.configuration_settings["prompt_prefix"].get(),
                ],
            ...
            for line in iter(self.process.stdout.readline, ""):
                if len(line) > 0:
                    ...
                    if "BOT:" in line:
                        self.status_light_canvas.itemconfig(
                            self.status_light_circle, fill="orange", outline=""
                        )
                    else:
                        self.status_light_canvas.itemconfig(
                            self.status_light_circle, fill="green", outline=""
